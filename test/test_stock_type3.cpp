...
int main(int argc, char* argv[])
{
	...
	{
		char* tmpPtr;
		int tmp = strtol(argv[1], &tmpPtr, 10);
		if(tmpPtr == argv[1])
		{
			cout << "Failed to parse " << argv[1] << " as tree level" << endl;
			return -1;
		}
		else
		{
			cout << "Using tree level: " << tmp << endl;
			treeLevel = tmp;
		}
	}
	...
	{
		ga.insert(gep_rand_chro(randSet, treeLevel, inputSize));
	}
